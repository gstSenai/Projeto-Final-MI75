    ...
    return (
        <div className="flex justify-center max-sm:text-lg max-md:text-xl max-lg:text-xl lg:text-xl w-full leading-normal text-black hover:text-white">
            <button
                type={type}
                onClick={onClick}
                className={`${montserrat.className} bg-vermelho bg-opacity-40 hover:bg-opacity-100 transition-all duration-300 ease-in-out shrink-0 text-center rounded-[20px] h-[50px] w-full ${className || ''}`}
            >
                {texto}
            </button>
        </div>
